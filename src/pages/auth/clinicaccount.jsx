...
function ClinicAccount() {
  ...
  const [dentalSoftwareList, setDentalSoftwareList] = useState([]);
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      updateOrganizationData("organizationId", currentUser.organization.org_id);
    } else {
      navigate("/signin");
    }
  }, []);
  ...
  async function fetchSoftwareList() {
    await Auth.getSoftwareList()
      .then((software) => {
        setDentalSoftwareList(software);
        updateOrganizationData("dentalSoftwareName",software?.[0]?.id)
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const data = await Auth.createclinic(clinic, currentUser);
    if (data) {
      handleSuccessModal({ clinicID: data?.clinic_id });
    }
  }
  function handleSuccessModal(data) {
    setModalContent({
      title: "Clinic Registration Success",
      data: { ...data, ...clinic },
    });
    setModalOpen(true);
  }
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateOrganizationData("clinicName", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="form-input w-full"
                        type="text"
                        placeholder="Appllo clinic"
                        value={clinic.clinicShortName}
                        onChange={(e) =>
                          updateOrganizationData(
                            "clinicShortName",
                            e.target.value
                    ...
                    <div>
                      <label
                        className="block text-sm font-medium mb-1"
                        htmlFor="dentalSoftwareName"
                      >
                        Dental Software Name
                      </label>
                      <select
                        id="role"
                      ...
                      >
                        {dentalSoftwareList?.map((software, index) => (
